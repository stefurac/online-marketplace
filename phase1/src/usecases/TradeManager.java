...
public class TradeManager {
    ...
    private HashMap<String, Trade> tradeIdToTrade = new HashMap<>();
    ...
    public void meetingConfirmed(String tradeId) {
        Trade trade = findTrade(tradeId);
        trade.setStatus(Trade.TradeStatus.CONFIRMED);
    }
    ...
    public void confirmTrade(String tradeId, String userId) {
        Trade trade = findTrade(tradeId);
        if (trade.getStatus().equals(Trade.TradeStatus.CONFIRMED) && trade.getRequester().equals(userId)) {
            trade.setStatus(Trade.TradeStatus.REQUESTERCONFIRMED);
        } else if (trade.getStatus().equals(Trade.TradeStatus.CONFIRMED) && trade.getReceiver().equals(userId)) {
            trade.setStatus(Trade.TradeStatus.RECEIVERCONFIRMED);
        } else if (trade.getStatus().equals(Trade.TradeStatus.REQUESTERCONFIRMED) ||
                trade.getStatus().equals(Trade.TradeStatus.RECEIVERCONFIRMED)) {
    ...
    private boolean helperTradesInOneWeek(Trade trade, String userId) {
        if (trade.getTradeId().startsWith("@return")) {
            ...
            return false;
        } else if (trade.getStatus().equals(Trade.TradeStatus.REJECTED)) {
            ...
            return true;
        } else if (trade.getReceiver().equals(userId) && !trade.getStatus().equals(Trade.TradeStatus.REQUESTED)) {
            // if the person has a Trade (and has a state after accepted)
            return true;
        } else {
            return false;
        }
     ...
     * @param userId the user ID
     * @return false if the user ID has reached the tradeLimit
     */
    ...
    public ArrayList<String> getAllStatusTradeByUserId(String userId, String tradeStatus) {
        ...
        Trade.TradeStatus status = statusConverter(tradeStatus);
    ...
    private Trade.TradeStatus statusConverter(String tradeStatus) {
        HashMap<String, Trade.TradeStatus> hm = new HashMap<>();
    ...
    public ArrayList<String> getAllCompletedBorrowTradeByUserId(String userId) {
        ...
        for (Map.Entry<String, Trade> trade : hm.entrySet()) {
            if (trade.getValue().getStatus().equals(Trade.TradeStatus.COMPLETED)
    ...
    public HashMap<String, Trade> getAllOneWayTradeId(){
        HashMap<String, Trade> hmap = new HashMap<>();
        for (Map.Entry<String, Trade> trade : getTradeIdToTrade().entrySet()){
    ...
    /**
     * Returns all two way trade in TradeManager
     * @return a HashMap of tradeIds and Trade
     */
    public HashMap<String, Trade> getAllTwoWayTradeId(){
        HashMap<String, Trade> hmap = new HashMap<>();
        for (Map.Entry<String, Trade> trade : getTradeIdToTrade().entrySet()){
            if (!trade.getValue().isOneWay()){
                hmap.put(trade.getKey(),trade.getValue());
     ...
     */
    public Trade.TradeStatus getTradeStatusByTradeId(String tradeId) {
    ...
    public String createReverseTrade(String tradeId) {
        ...
        newTrade.setStatus(Trade.TradeStatus.ACCEPTED);
